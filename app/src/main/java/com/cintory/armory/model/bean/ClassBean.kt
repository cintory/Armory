...
class ClassBean {
  ...
  fun getClassImage(): String {
    return WarcraftLogsApi.IMAGE_HOST + "img/warcraft/icons/${this.name!!.replace(" ", "")}.jpg"
  }
  ...
  inner class SpecsEntity {
    ...
    fun getSpecsImage(className: String): String {
      return WarcraftLogsApi.IMAGE_HOST + "img/warcraft/icons/${className.replace(" ", "")}-${this.name!!.replace(" ", "")}.jpg"
    }
  }
}