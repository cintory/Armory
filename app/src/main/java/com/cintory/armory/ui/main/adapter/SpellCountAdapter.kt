...
class SpellCountAdapter(t: MutableList<TableDetailBean>) :
  ...
  fun addNewsCalculateMax(news: List<TableDetailBean>) {
    news.forEach {
  ...
  fun clearAndAddNewsCalculateMax(news: List<TableDetailBean>) {
    maxTotal = -1
    news.forEach {
      if (maxTotal < it.total)
        maxTotal = it.total
  ...
  override fun getBindViewHolder(holder: SpellCountHolder, position: Int) {
    ...
    val percent = items[position].total * 100 / maxTotal
